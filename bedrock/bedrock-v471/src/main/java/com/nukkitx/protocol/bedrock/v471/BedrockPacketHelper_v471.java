package com.nukkitx.protocol.bedrock.v471;

import com.nukkitx.network.VarInts;
import com.nukkitx.protocol.bedrock.data.SoundEvent;
import com.nukkitx.protocol.bedrock.data.event.CodeBuilderActionEventData;
import com.nukkitx.protocol.bedrock.data.event.CodeBuilderScoreboardEventData;
import com.nukkitx.protocol.bedrock.data.event.TargetBlockHitEventData;
import com.nukkitx.protocol.bedrock.v465.BedrockPacketHelper_v465;
import io.netty.buffer.ByteBuf;

import static com.nukkitx.protocol.bedrock.data.inventory.stackrequestactions.StackRequestActionType.*;

public class BedrockPacketHelper_v471 extends BedrockPacketHelper_v465 {

    protected BedrockPacketHelper_v471(){
        super();

        // TODO register writers
    }

    @Override
    protected void registerLevelEvents() {
        super.registerLevelEvents();
    }

    @Override
    protected void registerSoundEvents() {
        super.registerSoundEvents();

        this.soundEvents.put(365, SoundEvent.SCLUK_CATALYST_BLOOM);
    }

    @Override
    protected void registerStackActionRequestTypes() {
        this.stackRequestActionTypes.put(0, TAKE);
        this.stackRequestActionTypes.put(1, PLACE);
        this.stackRequestActionTypes.put(2, SWAP);
        this.stackRequestActionTypes.put(3, DROP);
        this.stackRequestActionTypes.put(4, DESTROY);
        this.stackRequestActionTypes.put(5, CONSUME);
        this.stackRequestActionTypes.put(6, CREATE);
        this.stackRequestActionTypes.put(7, LAB_TABLE_COMBINE);
        this.stackRequestActionTypes.put(8, BEACON_PAYMENT);
        this.stackRequestActionTypes.put(9, MINE_BLOCK);
        this.stackRequestActionTypes.put(10, CRAFT_RECIPE);
        this.stackRequestActionTypes.put(11, CRAFT_RECIPE_AUTO);
        this.stackRequestActionTypes.put(12, CRAFT_CREATIVE);
        this.stackRequestActionTypes.put(13, CRAFT_RECIPE_OPTIONAL);
        this.stackRequestActionTypes.put(14, CRAFT_REPAIR_AND_DISENCHANT);
        this.stackRequestActionTypes.put(15, CRAFT_LOOM);
        this.stackRequestActionTypes.put(16, CRAFT_NON_IMPLEMENTED_DEPRECATED);
        this.stackRequestActionTypes.put(17, CRAFT_RESULTS_DEPRECATED);
    }

    public TargetBlockHitEventData readBlockHit(ByteBuf buffer) {
        int redstoneLevel = VarInts.readInt(buffer);
        return new TargetBlockHitEventData(redstoneLevel);
    }

    public void writeBlockHit(ByteBuf buffer, TargetBlockHitEventData eventData) {
        VarInts.writeInt(buffer, eventData.getRedstoneLevel());
    }

    public CodeBuilderActionEventData readCodeBuilderAction(ByteBuf buffer) {
        String action = readString(buffer);
        return new CodeBuilderActionEventData(action);
    }

    public void writeCodeBuilderAction(ByteBuf buffer, CodeBuilderActionEventData eventData) {
        writeString(buffer, eventData.getAction());
    }

    public CodeBuilderScoreboardEventData readCodeBuilderScoreboard(ByteBuf buffer) {
        String objectiveName = readString(buffer);
        int score = VarInts.readInt(buffer);
        return new CodeBuilderScoreboardEventData(objectiveName, score);
    }

    public void writeCodeBuilderScoreboard(ByteBuf buffer, CodeBuilderScoreboardEventData eventData) {
        writeString(buffer, eventData.getObjectiveName());
        VarInts.writeInt(buffer, eventData.getScore());
    }

}
